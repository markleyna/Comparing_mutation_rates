...
Genes <- read.csv("workingCBSalgaps.csv", stringsAsFactors = FALSE)
names(Genes)
nrow(Genes)
...
SalDNA<-gsub("\n","",SalDNA)
Genes$salgeneseq<-substring(SalDNA,first = Genes$salgapstart,last = Genes$salgapend)
...
G3<-data.frame(Genes$salgeneseq)
n=1
