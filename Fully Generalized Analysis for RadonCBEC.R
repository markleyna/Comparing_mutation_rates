...
#  while(n<=20){ #testing line when not running full version of code
  ...
  fileConn<-file("FastaInCBEC.txt") #reset the file to being blank text again
  writeLines("\n",fileConn)
  close(fileConn)
  ...
  alnlines<-readLines("FastaInCBEC.aln")
...
write.csv(Gene_vector_of_lengths,file= "CBECGeneData.csv")
